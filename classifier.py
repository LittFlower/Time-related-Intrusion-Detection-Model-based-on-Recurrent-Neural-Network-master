...
# 拟合及预测
history = classifier.fit(train_generator, epochs=250, verbose=2, steps_per_epoch=168,
...
train_probabilities = classifier.predict(train_generator, verbose=1, steps=len(train_generator))

train_pred = train_probabilities > 0.5
train_label_original = train_label_[(time_steps-1):-2, :]

test_probabilities = classifier.predict(test_generator, verbose=1, steps=len(test_generator))
